...
//  - short: R3 lands inside the sprint window after the real push (that push
//    was the first tap): OFF1 (withhold Up), then normal emission, so the
//    hold is the second tap;
//  - full: otherwise, or once a sequence has already produced a sprint:
//    OFF1, ON1 (first tap), OFF2, then normal emission (second tap).
...
typedef struct {
	...
	unsigned int up_on_ms;      // when Up last went false -> true
} LItPadDashState;
