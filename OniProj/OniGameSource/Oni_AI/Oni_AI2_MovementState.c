										   ...
										   PHtNode *inNode, UUtBool inActiveTransition)
{
	...
	if (oniAiTraceEnabled()) UUrStartupMessage("[GRID-DBG] SetParams char=%s success=%d node=%p room=%p gridXY=(%dx%d) pinStart=(%.2f,%.2f,%.2f) pinEnd=(%.2f,%.2f,%.2f)",
	...
	if (success) {
		...
		if (oniAiTraceEnabled()) UUrStartupMessage("[GRID-DBG] Generate char=%s success=%d grid_num_points=%d active_transition=%d grid_failure=%d",
	...
	if (!success) {
		if (oniAiTraceEnabled()) UUrStartupMessage("[GRID-DBG] SetupPath-FAIL char=%s about to ClearPath", ioCharacter->player_name);
