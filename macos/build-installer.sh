#!/usr/bin/env bash
# Assembles "Oni Texture Installer.app" (#20, #124) from the Swift sources plus
...
# Produces: $BINARY_DIR/bin/Oni Texture Installer.app
set -euo pipefail
...
SRC="$SOURCE_DIR/tools/OniTextureInstaller"
APP="$BINARY_DIR/bin/Oni Texture Installer.app"
...
# 1. Swift binary. Every *.swift in $SRC, main.swift last: top-level code
#    lives there, so no -parse-as-library.
swiftc -O -target arm64-apple-macosx15.0 \
    $(ls "$SRC"/*.swift | grep -v /main.swift) "$SRC/main.swift" \
    -framework AppKit -framework UniformTypeIdentifiers \
    -o "$MACOS_DIR/OniTextureInstaller"
...
cp "$SOURCE_DIR/macos/assets/Oni.icns" "$RESOURCES/OniTextureInstaller.icns"
...
codesign "${SIGN_ARGS[@]}" "$MACOS_DIR/OniTextureInstaller"
